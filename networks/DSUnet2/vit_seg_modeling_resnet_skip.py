...
class ResNetV2(nn.Module):
    ...
    def __init__(self, block_units, width_factor):
        ...
        self.merge1 = nn.Conv2d(in_channels=2 * 64, out_channels=64, kernel_size=1)
        self.edgeCov = nn.Conv2d(in_channels=3, out_channels=64, kernel_size=7, stride=2, padding=3)

    def forward(self, x):
        features = []
        b, c, in_size, _ = x.size()
        xH = F.conv2d(x,weight=my_Hfilter())
        xV = F.conv2d(x,weight=my_Vfilter())
        xT = torch.sqrt(xH * xH + xV * xV)
        xT = self.edgeCov(xT)
        x = self.root(x)
