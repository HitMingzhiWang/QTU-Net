...
class eca_layer_two(nn.Module):
    def __init__(self, channel, k_size):
        ...
        self.k_size = k_size
        self.conv = nn.Conv2d(channel, 16, kernel_size=k_size, bias=False, groups=1)
...
class SKConv(nn.Module):
    def __init__(self, features, M, G, r, stride=1, L=16):
        ...
        self.conv0_1 = nn.Conv2d(features, features, kernel_size=(7, 1), padding=(0, 3))
        self.conv0_2 = nn.Conv2d(features, features, kernel_size=(1, 7), padding=(3, 0))
        self.conv1_1 = nn.Conv2d(features, features, kernel_size=3, padding=1)
        self.conv2_1 = nn.Conv2d(features, features, kernel_size=7, padding=9, dilation=3)
...
class AttentionModule(nn.Module):
    def __init__(self, dim):
        ...
        self.conv_spatial = nn.Conv2d(2 * dim, dim, 3, stride=1, padding=3, dilatation=3)
...
class SpatialAttention(nn.Module):
    def __init__(self, d_model):
        ...
        self.proj_1 = nn.Conv2d(d_model, d_model, kernel_size=3, stride=1,padding=1)
...
class Encoder(nn.Module):
    def __init__(self, img_size=256, in_chans=3, depths=[3, 4, 6, 3], num_stages=4, drop_rate=0., drop_path_rate=0.,
        ...
        self.eca = eca_layer_two(channel=28,k_size=1)
        self.depths = depths
    ...
    def forward_features(self, x):
        ...
        cA, (cH, cV, cD) = dwt2(x_numpy, 'haar')
        cA = np.uint8(cA / np.max(cA) * 255)
        cH = np.uint8(cA / np.max(cH) * 255)
        cV = np.uint8(cA / np.max(cV) * 255)
        cD = np.uint8(cA / np.max(cD) * 255)
        c = np.concatenate((cV,cH,cA,cD),axis=1)
        c_tensor = torch.from_numpy(c).cuda()
        B = x.shape[0]
        # x = patch_embed(x)
        featureMap = []
    ...
    def __init__(self, img_size=256, depths=[3, 4, 6, 3], num_stages=3, drop_rate=0., drop_path_rate=0.,
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                ...
                for j in range(depths[i])])
            norm = norm_layer(embed_dims[i])
            covCat = nn.Conv2d(in_channels=embed_dims[i] * 2, out_channels=embed_dims[i], stride=1, kernel_size=3,padding=1)
            cur += depths[i]
            setattr(self, f"patch_embed{i + 1}", patch_embed)
            setattr(self, f"block{i + 1}", block)
            setattr(self, f"norm{i + 1}", norm)
