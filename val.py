...
if args.dataset == "WHDLD":
    args.volume_path = os.path.join(args.volume_path, "val_npz")
...
def inference(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        logging.info(
            'idx %d case %s mean_dice %f mean_hd95 %f mean_com %f mean_cor %f mean_q %f mean_f1 %f mean_iou %f mean_acc %f' % (
                i_batch, case_name, np.mean(metric_i, axis=0)[0], np.mean(metric_i, axis=0)[1],
                np.mean(metric_i, axis=0)[2], np.mean(metric_i, axis=0)[3], np.mean(metric_i, axis=0)[4],
                np.mean(metric_i, axis=0)[5], np.mean(metric_i, axis=0)[6], np.mean(metric_i, axis=0)[7]))
    metric_list = metric_list / len(db_test)
    for i in range(1, args.num_classes):
        logging.info(
            'Mean class %d mean_dice %f mean_hd95 %f mean_com %f mean_cor %f mean_q %f mean_f1 %f mean_iou %f mean_acc %f' % (
                i, metric_list[i - 1][0], metric_list[i - 1][1], metric_list[i - 1][2], metric_list[i - 1][3],
                metric_list[i - 1][4], metric_list[i - 1][5], metric_list[i - 1][6], metric_list[i - 1][7]))
    ...
    logging.info(
        'Testing performance in best val model: mean_dice %f mean_hd95 %f mean_com %f mean_cor %f mean_q %f mean_f1 %f mean_iou %f mean_acc %f mean %f' % (
            performance, mean_hd95, com, cor, q, f1, iou, acc, (performance + com + cor + q + f1 + iou + acc) / 7))
...
if __name__ == "__main__":
    ...
    dataset_config = {
        'WHDLD': {
            ...
            'volume_path': './datasets/WHDLD/val_npz',
        ...
        'WaterBodies': {
            'Dataset': Synapse_dataset,
            'volume_path': './datasets/WaterBodies/val_npz',
    ...
    #               num_classes=2).cuda()
    # net = qtnUnet(num_classes=2).cuda()
    # net = qtnUnet(img_size=256, in_channel=3, depth=[2, 2, 2, 2, 2], embed_dim=[16, 32, 64, 128, 256],
    #               num_classes=2).cuda()
    ...
    snapshot = os.path.join(args.output_dir, 'epoch_399.pth')
